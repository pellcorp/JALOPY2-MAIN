...
public class JavadocLexer extends InternalJavadocLexer implements Lexer {
	/** Logging. */
	private Logger _logger = LoggerFactory.getLogger("de.hunsicker.jalopy.language.javadoc");
	...
	/**
	 * Creates a new JavadocLexer object. Use {@link #setInputBuffer(Reader)} to
	...
	protected Token makeToken(int t) {
		...
		if (_mark != null) {
			...
			// ExtendedToken tok = new ExtendedToken(t,
	 ...
	 * space. Note that we assume you know what you do; we don't check if indeed
	 * newline chars were read!
	 *
	 * @param length
	 ...
	 * Inserts a newline.
	 *
	 * @param skipAllLeadingWhitespace
	 *            if <code>true</code>, all leading whitespace until the last
	 *            space before the first word will be removed; if
	 *            <code>false</code> only whitespace until and inclusive a
	 *            leading asterix will be removed.
	 *
	 * @see #makeToken
	 */
	 ...
	 */
	/*
	 * TODO private void consumeUntilWhitespace() { try { for (int type =
	 * LA(1);; type = LA(1)) { switch (type) { case ' ': case '\r': case '\n':
	 * case '\t': case EOF_CHAR: return; default: consume(); } } } catch
	 * (CharStreamException ignored) { } }
	 */
